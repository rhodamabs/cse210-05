...
class HandleCollisionsAction(Action):
    ...
    def execute(self, cast, script):
        ...
        if not self._is_game_over:
            ...
            self._handle_cycle_tail(cast)
            self._handle_game_over(cast)

    def _handle_cycle_tail(self, cast):
        """"Handles the cycles' tails
        ...
        cycle.cycle_tail(self._is_game_over)
        cycle1.cycle_tail(self._is_game_over)
    ...
    def _handle_segment_collision(self, cast):
        ...
        head_one = cycle.get_head()
        head_two = cycle1.get_head()
        # segments = snake.get_segments()[1:]
        segments_one = cycle.get_segments()[1:]
        segments_two = cycle1.get_segments()[1:]
    ...
    def _handle_game_over(self, cast):
        ...
        if self._is_game_over:
            ...
            segments_one = cycle.get_segments()
            segments_two = cycle1.get_segments()
            ...
            if score1.get_points() < 1 and score2.get_points() < 1:
                ...
                message.set_text(cycle1.get_name()+" wins!!!")
                cast.add_actor("messages", message)
            elif score2.get_points() < 1:
                message.set_text(cycle.get_name()+" win!!!")
                cast.add_actor("messages", message)
            else:
                cast.add_actor("messages", message)
